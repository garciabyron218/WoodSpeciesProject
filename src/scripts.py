...
for i, row in enumerate(table_rows):
    ...
    if len(cells) > 1:
        ...
        species_name = cells[3].text.strip().replace(" ", "_")
        if item_number and species_name:
            item_numbers.append((item_number, species_name))

print(f"Collected item numbers: {item_numbers}")

# Download images for the first 20 rows
for item_number, species_name in item_numbers:
    try:
        ...
        if response.status_code == 200:
            ...
            # Create species folder and save the image in the correct folder
            species_folder = os.path.join(".venv/Scripts/wood_species_images", species_name)
            os.makedirs(species_folder, exist_ok=True)
            image_path = os.path.join(species_folder, f"{item_number}.jpg")
